...
export interface DeliveredTest {
  ...
  instructions?: string;
  questions?: Question[];
  // Targeting configuration (who this test is delivered to)
  targetType?: 'all' | 'grade' | 'individual'; // 'all' (default), 'grade' (specific grades), 'individual' (specific student IDs)
  targetGrades?: number[]; // e.g. [1], [2], [3], [4]
  targetStudentIds?: string[]; // e.g. ["2026-001", "2026-002"]
}

export interface StudentRosterItem {
  studentId: string; // 学籍番号 (一意ID, e.g. "OT2401")
...
export interface QuizAttemptResult {
  id: string;
  testId?: string; // If taken from a delivered test
  testTitle: string;
  date: string;
  totalQuestions: number;
  correctCount: number;
  ...
  userAnswers: Record<string, number[]>; // questionId -> selected choices array (e.g. [1] or [2, 4])
  questionIds: string[];
  studentId?: string;
  grade?: number;
}
...
// Banned sample IDs that must never be shown or preserved
...
export const SAMPLE_TEST_IDS = new Set<string>([
  'test-1789208024231'
]);

export const SAMPLE_TEST_CODES = new Set<string>([
  'XCFTZL'
]);
...
export function isSampleTest(t: { id?: string; code?: string }): boolean {
  if (!t) return false;
  if (t.id && SAMPLE_TEST_IDS.has(t.id)) return true;
  if (t.code && SAMPLE_TEST_CODES.has(t.code)) return true;
  return false;
}
